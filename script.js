...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Load default template (Classic Academy)
  templateLibrary.loadTemplate("classic");
...
class CanvasEditor {
  ...
  drawLayer(layer) {
    ...
    layer.widthPx = metrics.width;
    layer.heightPx = size * (layer.lineHeight || 1.2);
    ...
    if (layer.colorType === "gradient") {
      ...
      const dx = Math.cos(angleRad) * (layer.widthPx * 0.5);
      const dy = Math.sin(angleRad) * (layer.heightPx * 0.5);
    ...
    if (layer.strokeEnabled) {
      ...
      this.ctx.strokeText(displayText, xPos, yPos);
    }

    // Draw main Text layout
    this.ctx.fillText(displayText, xPos, yPos);
...
class BulkProcessor {
  ...
  startRun() {
    ...
    // Clear old cache and pre-cache background template for active run resolution
    exportManager.bgCache = null;
    const res = exportManager.resolution;
    exportManager.getBgImageOnly(res, true);
...
class ExportManager {
  constructor() {
    ...
    drawLayerOnCanvas(tctx, layer, canvasW, canvasH, scale) {
      ...
      const metrics = tctx.measureText(displayText);
      const widthPx = metrics.width;
      const heightPx = size * (layer.lineHeight || 1.2);
      ...
      if (layer.strokeEnabled) {
        ...
        tctx.strokeText(displayText, xPos, yPos);
      }

      tctx.fillText(displayText, xPos, yPos);
      tctx.restore();
    }
